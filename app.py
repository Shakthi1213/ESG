...
def load_artifacts():
    ...
    "risk_model":     "esg_risk_model_sequential.pkl",   # 🔥 changed
    "scaler":         "scaler__1_.pkl",
    "score_features": "esg_score_features.pkl",
    "risk_features":  "esg_risk_features_sequential.pkl", # 🔥 changed
}
    artifacts = {}
...
def build_risk_input(predicted_esg_score: float):
    ...
    # 🔥 ESG injection (correct place)
    if "ESG_Overall" in row:
        row["ESG_Overall"] = predicted_esg_score
    elif "ESG_Score" in row:
        row["ESG_Score"] = predicted_esg_score
    elif "ESG_score" in row:
        row["ESG_score"] = predicted_esg_score
